...
class FeatureEngineer():
    ...
    def filter(self,std_thd=0.1,corr_thd=0.9,returnfeatures=True):
        ...
        #Normalize features and pvalue with standard scaler
        self.scaler = StandardScaler()
        c = self.df.select_dtypes("number").columns
        self.df[c] = self.scaler.fit_transform(self.df[c])

        des_df = self.df.drop(['smiles','value','assay'],axis=1)   
        des_df = self._low_std(des_df,std_thd)
        des_df = self._high_correlation(des_df,corr_thd)
        self.df = pd.concat([self.df[['smiles','value','assay']],des_df],axis=1,sort=False)
    ...
    def normalize(self,path=None,scaler='MinMax'):
        cols_to_norm = self.df.drop(['smiles','assay'],axis=1).columns
