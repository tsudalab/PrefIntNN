...
class IntegNet(object):
    def __init__(self,n_input,n_nodes1,n_nodes2,optim='Adam',weight_decay = 0,batch_size=100, n_epoch=200,learning_rate=0.001,cuda = True):
        self.n_input = n_input
        self.n_nodes1 = n_nodes1
        self.n_nodes2 = n_nodes2
    ...
    def predict_test(self,test_X,test_y,ndcgk=20):
        test_X = Variable(torch.from_numpy(test_X).float()).to(self.device)
        pred_y = self.predict(test_X)
        acc = ndcg(test_y,pred_y.cpu().numpy(),k=ndcgk)
